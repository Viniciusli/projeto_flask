...
fmt:              ## Format code using black & isort.
	$(ENV_PREFIX)isort projeto_flask/
	$(ENV_PREFIX)black -l 79 projeto_flask/
	$(ENV_PREFIX)black -l 79 tests/
	$(ENV_PREFIX)black -l 79 .github/templates/

.PHONY: lint
lint:             ## Run pep8, black, mypy linters.
	$(ENV_PREFIX)flake8 projeto_flask/
	$(ENV_PREFIX)flake8 .github/templates/
	$(ENV_PREFIX)black -l 79 --check projeto_flask/
	$(ENV_PREFIX)black -l 79 --check tests/
	$(ENV_PREFIX)black -l 79 --check .github/templates/
	$(ENV_PREFIX)mypy --ignore-missing-imports projeto_flask/

.PHONY: test
test: lint        ## Run tests and generate coverage report.
	$(ENV_PREFIX)pytest -v --cov-config .coveragerc --cov=projeto_flask -l --tb=short --maxfail=1 tests/
...
release:          ## Create a new tag for release.
	...
	@git tag $${TAG}
	@echo "$${TAG}" > projeto_flask/VERSION
	@$(ENV_PREFIX)gitchangelog > HISTORY.md
	@git add projeto_flask/VERSION HISTORY.md
...
switch-to-poetry: ## Switch to poetry package manager.
	...
	@rm -rf .venv
	@poetry init --name=projeto_flask --author=Viniciusli
